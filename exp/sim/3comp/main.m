...
% version control
%   2018-02-24      original
%   2018-02-28      modified for three-compartment case
...
addpath('./dess_1comp');                % needed here only for simulating 3-compartment dess data
addpath('..');
addpath('../../../model/spgr');
addpath('../../../model/dess');
addpath('../../../model/mese');
...
% header options
header.t1 = 1;                          % assign diff t1 to each comp but only assume bulk t1 known
...
for i = 1:dim.comp
  ...
  sm = sm + bsxfun(@times, x.(tmp{i+1}),...
    mese(x.tot, tmp1, tmp2, nu.kap, Pm.ex, Pm.ref, Pm.ncyc, Pm.tr, Pm.te,...
    'mask', mask.est,...
    'bool.mag', 0));
end
...
sd = zeros(dim.x, dim.y, Sd.de, Ed.de);
tmp = fieldnames(x);
for i = 1:dim.comp
  if header.t1
    tmp1 = bsxfun(@times, comp.xnom.t1(i), ones(dim.x, dim.y));
  ...
  tmp2 = bsxfun(@times, comp.xnom.t2(i), ones(dim.x, dim.y));
  for s = 1:Sd.de
    [tmp3, tmp4] = dess_fun_v2(...
      x.tot, tmp1, tmp2,...
      Pd.de.aex(s), Pd.de.tr(s), Pd.de.te(s,1), Pd.de.te(s,2),...
      'mask', mask.est,...
      'kap', nu.kap,...
      'mag', 0);
    sd(:,:,s,1) = sd(:,:,s,1) + bsxfun(@times, x.(tmp{i+1}), tmp3);
    sd(:,:,s,2) = sd(:,:,s,2) + bsxfun(@times, x.(tmp{i+1}), tmp4);
  end
end
...
if header.im
  ...
  for i = 1:length(tmp)
    for j = 1:length(opt.type)
      tmp2 = cat(3, x.(tmp{i}), xhat.mese(1).(tmp{i}), xhat.mese(2).(tmp{i}));
      if strcmp(opt.type{j}, 'err')
        tmp2 = bsxfun(@minus, tmp2, x.(tmp{i}));
        tmp2 = abs(tmp2);
      end
  ...
  for j = 1:length(opt.type)
    ...
    hold on;
    if strcmp(opt.type{j}, 'im')
      text(dim.x/2-1:dim.x:9*dim.x/2, zeros(5,1), col(opt.im.both),...
        'VerticalAlignment', 'bottom',...
        'HorizontalAlignment', 'center',...
        'FontSize', 11,...
        'Color', 'k');
...
for r = 1:length(opt.roi)
  for m = 1:length(opt.im.both)
    ...
    summ(m,r) = stat(...
      ...
      'true', mean(masker(x.mw, mask.(opt.roi{r}))));
    if header.st
